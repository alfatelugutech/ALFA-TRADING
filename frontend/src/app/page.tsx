...
export default function Home() {
  ...
  const [paperInfo, setPaperInfo] = useState<any | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(backendUrl + "/status/all")
      ...
      .then((d)=>{
        ...
        if (d?.paper) setPaperInfo(d.paper);
        if (d?.funds) setFunds(d.funds);
      }).catch(()=>{});
    // Prefetch PnL for balances widget
    fetch(backendUrl + "/pnl").then(r=>r.json()).then((p)=>{ try { setPnlState(p); } catch {} }).catch(()=>{});
  }, []);
    ...
    <main className={dark ? "dark" : ""} style={{ maxWidth: 1000, margin: "20px auto", fontFamily: "system-ui, sans-serif" }}>
      ...
      <section className="card" style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          <div>
            <div style={{ color: "#666", fontSize: 12 }}>Paper Cash</div>
            <div style={{ fontWeight: 700 }}>₹{(paperInfo?.cash ?? 0).toFixed(2)}</div>
          </div>
          <div>
            <div style={{ color: "#666", fontSize: 12 }}>Paper Equity</div>
            <div style={{ fontWeight: 700 }}>₹{(paperInfo?.equity ?? 0).toFixed(2)}</div>
          </div>
          <div>
            <div style={{ color: "#666", fontSize: 12 }}>Paper PnL</div>
            <div style={{ fontWeight: 700, color: (pnlState?.paper?.total ?? 0) >= 0 ? "#0a0" : "#a00" }}>₹{(pnlState?.paper?.total ?? 0).toFixed(2)}</div>
          </div>
          <div>
            <div style={{ color: "#666", fontSize: 12 }}>Live PnL</div>
            <div style={{ fontWeight: 700, color: (pnlState?.live?.total ?? 0) >= 0 ? "#0a0" : "#a00" }}>₹{(pnlState?.live?.total ?? 0).toFixed(2)}</div>
          </div>
          <div>
            <div style={{ color: "#666", fontSize: 12 }}>Live Cash</div>
            <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.available?.cash || 0).toFixed(2)}</div>
          </div>
        </div>
      </section>
      {funds?.equity && (
        <section className="card" style={{ marginBottom: 12 }}>
          <h3>Live Funds (Equity)</h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Net</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.net || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Avail Cash</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.available?.cash || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Opening Bal</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.available?.opening_balance || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Collateral</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.available?.collateral || funds?.equity?.available?.live_balance || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Intraday Payin</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.available?.intradaypayin || 0).toFixed(2)}</div>
            </div>
          </div>
          <div style={{ marginTop: 8, color: "#666", fontSize: 12 }}>Utilised</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12 }}>
            {[
              ["debits", funds?.equity?.utilised?.debits],
              ["exposure", funds?.equity?.utilised?.exposure],
              ["span", funds?.equity?.utilised?.span],
              ["option premium", funds?.equity?.utilised?.option_premium],
              ["holding sales", funds?.equity?.utilised?.holding_sales],
              ["turnover", funds?.equity?.utilised?.turnover],
            ].map(([label, val]: any, i: number) => (
              <div key={`util-${i}`}>
                <div style={{ color: "#666", fontSize: 12 }}>{label as string}</div>
                <div style={{ fontWeight: 700 }}>₹{Number(val || 0).toFixed(2)}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>MTM Realised</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.utilised?.m2m_realised || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>MTM Unrealised</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.equity?.utilised?.m2m_unrealised || 0).toFixed(2)}</div>
            </div>
          </div>
        </section>
      )}
      {funds?.commodity && Object.keys(funds?.commodity || {}).length > 0 && (
        <section className="card" style={{ marginBottom: 12 }}>
          <h3>Live Funds (Commodity)</h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Net</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.commodity?.net || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Avail Cash</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.commodity?.available?.cash || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Exposure</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.commodity?.utilised?.exposure || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ color: "#666", fontSize: 12 }}>Span</div>
              <div style={{ fontWeight: 700 }}>₹{Number(funds?.commodity?.utilised?.span || 0).toFixed(2)}</div>
            </div>
          </div>
        </section>
      )}
      ...
      <div className="card" style={{ marginBottom: 12, backgroundColor: "#f8f9fa", border: "2px solid #007bff" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
          ...
          <a href="/reports/portfolio" style={{ 
            padding: "12px 16px", 
